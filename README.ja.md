...
2. RuaKey を通常どおり開きます。**Developer ID 署名と Apple 公証**済みのリリースは Gatekeeper を通過します。旧 ad-hoc ビルドのみ、初回に**右クリック → 開く**が必要な場合があります。
3. 求められたら**入力モニタリング**を許可し、**一度終了して再起動**——macOS の仕様により、再起動しないと有効になりません。

正式署名版は RuaKey 内蔵アップデーターからインストールされ、App の識別情報が維持されるため、入力モニタリングを再許可する必要はありません。旧 ad-hoc ビルドから更新しても拒否されたままの場合のみ、**システム設定 ▸ プライバシーとセキュリティ ▸ 入力モニタリング**で古い RuaKey 項目を一度削除し、現在の App を追加してください。
