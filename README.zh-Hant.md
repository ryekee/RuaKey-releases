...
2. 正常開啟 RuaKey。經過 **Developer ID 簽署和 Apple 公證**的版本可直接通過 Gatekeeper；只有舊 ad-hoc 版本可能仍需執行一次**右鍵 → 開啟**。
3. 依提示授予**輸入監控**權限，然後**結束並重啟一次**——macOS 要求重啟 App 才能生效。

正式簽署版會透過 RuaKey 內建更新器安裝，並保持相同的 App 身分，不應再次要求輸入監控權限。如果你從舊 ad-hoc 版本升級後權限仍被拒絕，請在**系統設定 ▸ 隱私權與安全性 ▸ 輸入監控**中移除舊 RuaKey 項目一次，再加入目前的 App。
